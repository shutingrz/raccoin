...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 8882
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 8883
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18882
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19883
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 20887
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 20888
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 8882
	%{_sbindir}/semanage port -d -p tcp 8883
	%{_sbindir}/semanage port -d -p tcp 18882
	%{_sbindir}/semanage port -d -p tcp 19883
	%{_sbindir}/semanage port -d -p tcp 20887
	%{_sbindir}/semanage port -d -p tcp 20888
	for selinuxvariant in %{selinux_variants}; do
