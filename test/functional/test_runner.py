...
def main():
    ...
    tmpdir = "%s/raccoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and raccoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "raccoind"]) is not None:
            print("%sWARNING!%s There is already a raccoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "RACCOIND" not in os.environ:
        os.environ["RACCOIND"] = build_dir + '/src/raccoind' + exeext
        os.environ["RACCOINCLI"] = build_dir + '/src/raccoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `raccoin-cli help` (`rpc_interface.txt`).
